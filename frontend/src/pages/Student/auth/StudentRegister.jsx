...
function Register() {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Error handled by reduxError sync
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-50 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg text-center">
          <div className="mb-8 inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-bold uppercase tracking-wider">
            ...
            <span>Join our learning platform</span>
          </div>
          <h2 className="text-4xl font-extrabold mb-6 leading-tight">
            Start Your Learning <span className="text-blue-600">Journey</span> Today
          </h2>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            <div className="flex items-center space-x-2 mb-2">
              <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-bold">
                Student Account
              </span>
            </div>
          ...
          <div className="flex items-center space-x-2 mb-8 px-1">
            <div className={`h-1.5 flex-1 rounded-full ${isOtpSent ? 'bg-blue-600' : 'bg-blue-600'}`}></div>
            <div className={`h-1.5 flex-1 rounded-full ${isVerified ? 'bg-blue-600' : isOtpSent ? 'bg-blue-200 animate-pulse' : 'bg-slate-200'}`}></div>
            <div className={`h-1.5 flex-1 rounded-full ${isVerified ? 'bg-blue-200 animate-pulse' : 'bg-slate-200'}`}></div>
          </div>
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-5">
            ...
            {!isOtpSent ? (
              <button
                ...
                disabled={loading}
                className={`w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-600/20 transition-all transform active:scale-[0.98] flex items-center justify-center space-x-2 group ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
                <span>{loading ? "Sending Code..." : "Verify Email"}</span>
              ...
              <div className="space-y-5 animate-slideUp">
                <div className="space-y-2">
                  <div className="flex justify-between items-center px-1">
                    ...
                    <button type="button" onClick={() => setIsOtpSent(false)} className="text-blue-600 text-xs font-bold hover:underline">Change Email</button>
                  </div>
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-blue-600 transition-colors">
                      <ShieldCheck size={18} />
                    ...
                    <input
                      ...
                      onChange={formik.handleChange}
                      className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-slate-200 bg-white outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 text-center tracking-[0.5em] font-bold text-xl transition-all"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className={`w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-600/20 transition-all transform active:scale-[0.98] ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
            ...
            {isVerified && (
              <div className="space-y-5 animate-slideUp">
                <div className="bg-green-50 text-green-700 p-3 rounded-xl text-xs flex items-center mb-2 font-medium border border-green-100">
                  <CheckCircle2 size={16} className="mr-2" /> Email verified successfully
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                      className="w-full pl-11 pr-12 py-3.5 rounded-xl border border-slate-200 bg-slate-50 outline-none focus:border-blue-600 focus:bg-white transition-all"
                    />
                    <button
                      type="button"
                ...
                <button
                  type="submit"
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-600/20 transition-all transform active:scale-[0.98]"
                >
